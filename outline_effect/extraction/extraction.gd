...
func _init() -> void:
    ...
    nearest_sampler = rd.sampler_create(nearest_sampler_state)
...
func _render_callback(_effect_callback_type: int, render_data: RenderData) -> void:
    ...
    for view in view_count:
        ...
        stencil_uniform.add_id(linear_sampler)
        stencil_uniform.add_id(stencil_effect.output_texture.texture_rd_rid)
        #endregion

        var uniform_set: RID = UniformSetCacheRD.get_cache(shader, 0, [scene_data_uniform, color_uniform, depth_uniform, normal_uniform, stencil_uniform])
