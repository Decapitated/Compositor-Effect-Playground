@tool
class_name StencilCompositorEffect extends CompositorEffect

const StencilShaderPath: String = "res://sophisticated_outline/stencil/stencil.glsl"
...
func _build_shader():
    ...
    # load the shader
    var shader_source = _load_glsl_from_file(StencilShaderPath)
    if not shader_source:
        push_error("failed to load shader source: ", StencilShaderPath)
        return
